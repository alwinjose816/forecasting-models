...
def run_xgboost_forecast_DRFIv2(product):
    ...
    drfi_df["date"] = (
        daily.iloc[split_index:]
        ["date"]
        .astype(str)
        .values[:len(drfi_df)]
    )
    lowest = decile_error.iloc[0][
    "forecast_error"
    ]

    highest = decile_error.iloc[-1][
        "forecast_error"
    ]
    ...
    return {
        ...
        "drfi_values":
        drfi_df["drfi_v2"].tolist(),
       

        "error_values":
        forecast_error.tolist(),
        "decile_error":
        decile_error.to_dict("records"),
        "daily_drfi":
        drfi_df[
            [
                "date",
...
if __name__ == "__main__":
    ...
    result = run_xgboost_forecast_DRFIv2(
        "Overall"
    )
