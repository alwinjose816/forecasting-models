...
def run_transition_aware_xgboost(product):
    ...
    forecast_split = transition_result["split_index"]

    transition_matrix = pd.DataFrame(
        transition_result["transition_matrix"]
    )
    ...
    stability_map = transition_result[
        "stability_map"
    ]

    expected_duration_map = transition_result[
        "expected_duration_map"
    ]

    transition_entropy_map = transition_result[
        "transition_entropy_map"
    ]
    ...
    for i in range(30):
        ...
        row_probs = transition_matrix.loc[
            current_state
        ]
        ...
        row["state_stability"] = float(
            stability_map[str(next_state)]
        )

        row["expected_duration"] = float(
            expected_duration_map[str(next_state)]
        )

        row["transition_entropy"] = float(
            transition_entropy_map[str(next_state)]
        )

        for col in state_cols:
            row[col] = 0

        row[f"state_{next_state}"] = 1
...
if __name__ == "__main__":
    ...
    result = run_transition_aware_xgboost(
        "Overall"
    )
